        ...
        for expected in target_neighbors:
            ...
            if t_ip in neighbors_data:
                ...
                if actual_state == expected["state"] or actual_state.isdigit():
                    logger.info(
                        f"{device_name}: [PASS] BGP Neighbor {t_ip} Established"
                    )
                else:
                    logger.error(
                        f"{device_name}: [FAIL] BGP Neighbor {t_ip} is {actual_state}"
                    )
            else:
        ...
        for rule in GLOBAL_RULES["ospf"]["neighbors"]:
            ...
            if n_id in active_neighbors:
                if rule["state"].lower() in active_neighbors[n_id].lower():
                    logger.info(
                        f"{device_name}: [PASS] OSPF Neighbor {n_id} {rule['state']}"
                    )
                else:
                    logger.error(
                        f"{device_name}: [FAIL] OSPF Neighbor {n_id} state mismatch"
                    )
            else:
